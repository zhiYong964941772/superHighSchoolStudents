...
+ (void)resolveArrt:(NSArray *)arrt{
    ...
    // 拼接属性字符串代码
    NSMutableString *strM = [NSMutableString string];
    // 1.遍历数组，把数组中的所有idx取出来，生成对应的属性代码
    [arrt enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        // 类型经常变，抽出来
        NSString *type;
        ...
        NSString *str;
        if ([type containsString:@"NS"]) {
            str = [NSString stringWithFormat:@"@property (nonatomic, copy) %@ *test%ld;",type,idx];
        }else{
            str = [NSString stringWithFormat:@"@property (nonatomic, assign) %@ test%ld;",type,idx];
        }
